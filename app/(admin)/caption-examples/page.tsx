...
import DeleteCaptionExampleButton from "@/components/DeleteCaptionExampleButton";

export default async function CaptionExamplesPage() {
  const supabase = await createClient();
  const { data: captionExamples, error } = await supabase
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Caption Examples</h1>
          <p className="text-slate-500 text-sm mt-1">
            {captionExamples?.length ?? 0} caption examples
          </p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {captionExamples?.map((example, i) => (
              <tr
                key={example.id}
                className={i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}
              >
                <td className="px-5 py-3 font-mono text-slate-400 text-xs">
                  {String(example.id).slice(0, 8)}…
                </td>
                <td className="px-5 py-3 text-slate-600 max-w-[300px] truncate">
                  {example.content ?? "—"}
                </td>
                <td className="px-5 py-3 font-mono text-slate-400 text-xs">
                  {example.image_id ? String(example.image_id).slice(0, 8) + "…" : "—"}
                </td>
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <Link
                      href={`/caption-examples/${example.id}/edit`}
                      className="text-slate-600 hover:text-violet-600 text-xs font-medium"
                    >
                      Edit
                    </Link>
                    <DeleteCaptionExampleButton id={String(example.id)} />
                  </div>
                </td>
              </tr>
            ))}
            {(!captionExamples || captionExamples.length === 0) && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-slate-400">
                  No caption examples yet.{" "}
                  <Link href="/caption-examples/new" className="text-violet-600 hover:underline">
                    Add one.
                  </Link>
                </td>
              </tr>
